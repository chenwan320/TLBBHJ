...
echo "
+----------------------------------------------------------------------
| CentOS 8.x 天龙服务端环境 安装脚本
+----------------------------------------------------------------------
| Copyright © 1993-2022 乔安 版权所有.
+----------------------------------------------------------------------
...
GetSysInfo(){
    ...
    if [ "${is64bit}" != '64' ];then
        echo "错误提示: 当前脚本只能在 CentOS 8.x 64位系列版本上运行"
        exit;
    fi
    isPy26=$(python -V 2>&1|grep '2.6.')
    if [ "${isPy26}" ];then
        echo "错误提示: 当前脚本只能在 CentOS 8.x 系列版本上运行"
        exit;
    fi
}
